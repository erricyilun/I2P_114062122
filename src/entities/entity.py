...
class Entity:
    ...
    def camera(self) -> PositionCamera:
        ...
        new_position = PositionCamera(self.position.x, self.position.y)
        new_position.x -= GameSettings.SCREEN_WIDTH / 2
        new_position.y -= GameSettings.SCREEN_HEIGHT / 2


        return PositionCamera(int(new_position.x), int(new_position.y))
