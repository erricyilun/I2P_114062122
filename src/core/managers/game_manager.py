...
from src.maps.map import Map, MiniMap
...
class GameManager:
    ...
    def __init__(self, maps: dict[str, Map], start_map: str, 
        ...
        self.mini_map = MiniMap(maps[start_map].path_name, scale=0.06)
    ...
    def try_switch_map(self) -> None:
        if self.should_change_scene:
            ...
            if self.mini_map:
                self.mini_map.update(self.current_map.path_name)
    ...
    def check_bush_collision(self, bush: pg.Rect) -> bool:
        if self.maps[self.current_map_key].check_bush_collision(bush):
            return True
        return False
